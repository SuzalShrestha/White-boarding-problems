...
let threeUniqueVowels = (string) => {
  let vowels = "aeiou";
  let result = [];
  for (let item of string) {
    if (vowels.includes(item) && !result.includes(item)) {
      result.push(item);
    }
  }
...
let vowelShift = (sentence) => {
  ...
  for (let item of sentence) {
    if (vowels.includes(item)) {
      result.push(vowels[(vowels.indexOf(item) + 1) % 5]);
    } else {
      result.push(item);
    }
  }
...
let numsToWords = (numString) => {
  ...
  for (let letter of numString) {
    result.push(words[letter]);
  }
  return result.join("");
};
let moreDotLessDash = (string) => {
  let countDot = 0;
  let countDash = 0;
  for (let item of string) {
    if (item === ".") countDot++;
